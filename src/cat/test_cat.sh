...
testing()
{
    ...
    then
      (( SUCCESS++ ))
        echo -e "$red$FAIL$no_color/$green$SUCCESS$no_color/$COUNTER $green success $no_color cat $t"
    else
      (( FAIL++ ))
        echo -e "$red$FAIL$no_color/$green$SUCCESS$no_color/$COUNTER $red fail $no_color cat $t"
    fi
